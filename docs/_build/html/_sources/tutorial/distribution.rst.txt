...
For the purpose of this model and work, we are going to assume that any
aerosol distribution can be described as the sum of :math:`n` lognormal
...
Thus, we need :math:`3n` parameters to describe any aerosol
distribution.

Using the ``opcsim.AerosolDistribution`` class, we can build our own
distributions by defining each aerosol mode as its own lognormal
distribution.

Ex: define an aerosol distribution with a single mode (:math:`N=1000`,
:math:`D_{pg}=100\;nm`, :math:`\sigma=1.5`)
...
.. math:: n_N^o(logD_p)=\frac{dN}{dlogD_p}=ln(10)\frac{N_t}{\sqrt{2\pi} \; ln\sigma_g}exp\Big(-\frac{(lnD_p - lnD_{pg})^2}{2ln^2\sigma_g}\Big)
...
.. math:: n_S^o(logD_p)=\pi D_p^2 n_N^o(logD_p)=\frac{dS}{dlogD_p}=ln(10)\frac{\pi D_p^2 N_t}{\sqrt{2\pi} \; ln\sigma_g}exp\Big(-\frac{(lnD_p - lnD_{pg})^2}{2ln^2\sigma_g}\Big)
...
.. math:: n_V^o(logD_p)=\frac{\pi}{6} D_p^3 n_N^o(logD_p)=\frac{dV}{dlogD_p}=ln(10)\frac{\pi D_p^3 N_t}{6\sqrt{2\pi} \; ln\sigma_g}exp\Big(-\frac{(lnD_p - lnD_{pg})^2}{2ln^2\sigma_g}\Big)
...
``log``, or ``log10``), the weight (``number``, ``surface``, or
``volume``), and an optional ``mode`` parameter in case you would like
...
``weight='number'``, ``base='log10'``). To calculate the number
probability for the urban aerosol distribution at 0.1 micron, we do the
following:
...
of #/cc. We can also calculate a whole range of values by providing an
array for the ``dp`` value:
...
matplotlib axis object which makes it extremely easy to build on top of
this.
...
.. image:: distribution_files/distribution_15_0.png
...
.. code:: ipython3
    ...
    marine = opcsim.load_distribution("Marine")
    rural = opcsim.load_distribution("Rural")
    rc = opcsim.load_distribution("Remote Continental")
    free_trop = opcsim.load_distribution("Free Troposphere")
    polar = opcsim.load_distribution("Polar")
    desert = opcsim.load_distribution("Desert")
    
    ax = opcsim.plots.pdfplot(urban, ax=ax)
    ax = opcsim.plots.pdfplot(marine, ax=ax)
    ax = opcsim.plots.pdfplot(rural, ax=ax)
    ax = opcsim.plots.pdfplot(rc, ax=ax)
    ax = opcsim.plots.pdfplot(free_trop, ax=ax)
    ax = opcsim.plots.pdfplot(polar, ax=ax)
    ax = opcsim.plots.pdfplot(desert, ax=ax, plot_kws={'linestyle': '--'})
...
The cumulative distribution function is just the integration of the PDF
between two particle diameters. The math for each weight is described
below.
...
.. code:: ipython3
    ...
    ax = opcsim.plots.cdfplot(urban, weight='volume')
